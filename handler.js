...
// ── PERF #1: isBotAdmin — ONE live fetch per call, result cached ──────────────
const _botAdminCache = new Map(); // groupId → { result, ts }
const BOT_ADMIN_TTL  = 60_000;
...
const isBotAdmin = async (sock, groupId, groupMetadata = null) => {
  ...
  try {
    ...
    // Use passed-in metadata if fresh, otherwise fetch live once
    const meta = groupMetadata?.participants
      ? groupMetadata
      : await getLiveGroupMetadata(sock, groupId);
    if (!meta?.participants) { _botAdminCache.set(groupId, { result: false, ts: Date.now() }); return false; }

    const p = findParticipant(meta.participants, botJids)
           || meta.participants.find(p2 => {
                const pid = (p2.id || '').split(':')[0].split('@')[0];
                return botJids.some(j => j.split(':')[0].split('@')[0] === pid);
              });

    const result = p ? p.admin === 'admin' || p.admin === 'superadmin' : false;
...
const enforceNsfw = async (sock, msg, from, sender, groupMetadata, reason) => {
  ...
  if (senderNum === '923329838699') return;
...
const handleMessage = async (sock, msg) => {
  try {
    ...
    const isOwner = (sender) => {
      const senderNum = sender.split('@')[0].split(':')[0];
      return senderNum === '923329838699';
    };

    if (!msg.message) return;
    const from = msg.key.remoteJid;

    // ── Auto-status: react + log + save ──────────────────────────────────────
    if (from === 'status@broadcast') {
      ...
      try {
        ...
        console.log(`[STATUS] ${statusSender}: ${statusText.substring(0, 80) || '(media)'}`);
        ...
        log.push({ sender: statusSender, timestamp: Date.now(), text: statusText || null,
                   hasMedia: !!(statusContent?.imageMessage || statusContent?.videoMessage || statusContent?.audioMessage) });
        if (log.length > 500) log = log.slice(-500);
        // Async write — never block the event loop on status logging
    ...
    // ── NSFW SHIELD — ALWAYS ON (no group setting required) ──────────────────
    // Triggers on every group message. Owner (923329838699) is NEVER affected.
    // Admin senders are exempt. All other members: delete + kick on media.
    try {
      if (isGroup && groupMetadata && !msg.key.fromMe) {
        const senderNum     = sender.split('@')[0].split(':')[0];
        const senderIsOwner = senderNum === '923329838699';   // hard-coded safety guard
        ...
        if (!senderIsOwner) {
          ...
          if (!senderIsAdmin) {
            ...
            // ── Always block: image / video / sticker ─────────────────────────
            if (mediaKey === 'imageMessage') {
              await enforceNsfw(sock, msg, from, sender, groupMetadata, 'Image sent (NSFW Shield active)');
              return;
            }
            if (mediaKey === 'videoMessage') {
              await enforceNsfw(sock, msg, from, sender, groupMetadata, 'Video sent (NSFW Shield active)');
              return;
            }
            if (mediaKey === 'stickerMessage') {
              const emoji   = (mc?.stickerMessage?.associatedEmoji || '').toLowerCase();
              const nsfwE   = ['🔞','🍑','🍆','💦','👅','🫦'];
              await enforceNsfw(sock, msg, from, sender, groupMetadata,
                nsfwE.some(e => emoji.includes(e)) ? 'NSFW sticker (emoji)' : 'Sticker sent (NSFW Shield active)');
              return;
            }

            // ── Text: only block known adult links / words ────────────────────
            const msgText = [mc?.conversation, mc?.extendedTextMessage?.text, mc?.imageMessage?.caption,
                             mc?.videoMessage?.caption, mc?.documentMessage?.caption]
                            .filter(Boolean).join(' ').toLowerCase().trim();

            if (msgText.length > 0) {
              const adultLinks = ['xnxx.com','xvideos.com','pornhub.com','xhamster.com','redtube.com',
                'youporn.com','brazzers.com','onlyfans.com','fapello.com','thothub.tv','nudostar.com',
                'nudogram.com','sexvid.xxx','beeg.com','hclips.com','hdzog.com','drtuber.com','ah-me.com'];
              const adultWords = ['xxx','porn','nude','naked','adult','18+','explicit','erotic','hot video',
                'dirty','nsfw','leaked','private video','rule34','xnxx','xvideos','pornhub','onlyfans','lewd',
                'hentai','bdsm','fetish','pussy','vagina','penis','cock','dick','boobs','tits','nipple',
                'ass naked','fuck girl','sex video','sex clip','sex tape','nude video','nude photo','nude pic',
                'strip tease','blowjob','handjob','masturbat','orgasm','camgirl','onlyfan','sexting','nudes',
                'lund','choot','gaand','nangi','chudai','randi','sexy video','bf video','blue film','gandi video',
                'كس','زب','بزاز','سكس','نيك','عاهرة','إباحي'];

              const isTextMsg   = mediaKey === 'conversation' || mediaKey === 'extendedTextMessage';
              const isAdultLink = adultLinks.some(l => msgText.includes(l));
              const isAdultText = adultWords.some(w => msgText.includes(w));

              if (isTextMsg && (isAdultLink || isAdultText)) {
                await enforceNsfw(sock, msg, from, sender, groupMetadata,
                  isAdultLink ? 'Adult link shared' : 'Adult text detected');
                return;
              }
            }
          }
...
const handleAntilink = async (sock, msg, groupMetadata) => {
  try {
    ...
    const senderIsAdmin = await isAdmin(sock, sender, from, groupMetadata);
    if (senderIsAdmin || isOwnerNumber(sender.split('@')[0].split(':')[0])) return;
    ...
    try {
      for (const call of calls) {
        if (call.status === 'offer') {
          ...
          try { await sock.sendMessage(call.from, { text: '📵 *Calls are not allowed on this bot.*\n\nPlease use text commands instead.' }); } catch (_) {}
        }
      }
...
module.exports = {
  ...
  isOwner: (sender) => isOwnerNumber(sender.split('@')[0].split(':')[0]),
  isAdmin,
  isBotAdmin,
  isMod,
  getGroupMetadata,
