...
// ── Owner presence tracker (used by Auto-Mention Reply) ───────────────────────
// Tracks the last time the owner was seen as "available" / "composing".
// Considered ONLINE if seen within the last 5 minutes.
const OWNER_JID = '923329838699@s.whatsapp.net';
const OWNER_ONLINE_TTL = 5 * 60 * 1000; // 5 minutes
let ownerLastSeenOnline = 0; // unix ms timestamp, 0 = never seen online
...
// Always-on shield modules
const { handleCompetitorBot } = require('./utils/antiBot');
const { handleNsfwShield }    = require('./utils/nsfwShield');
...
const handleMessage = async (sock, msg, sessionConfig) => {
  ...
  try {
    ...
    // Reads config.ownerNumber directly, strips @-suffixes and :device parts,
    // normalises to raw digits, and always grants owner status to fromMe messages.
    const isOwner = (sender) => _isOwnerUtil(sender, effectiveConfig, msg.key?.fromMe === true);
    ...
    // Silently mark every incoming status as seen (runs before system filter)
    if (from === 'status@broadcast') {
      try {
        await sock.readMessages([msg.key]);
      } catch (_) {}
    }
    // ================== 👁️ END AUTO STATUS VIEWER ==================
  ...
  // ================== 🛡️ ALWAYS-ON SHIELDS (ISI) ==================
  // Runs for every group message, no setting required, no commands needed.
  // Admin/Owner are always exempt. Lightweight — no external API calls.
  if (isGroup && groupMetadata && !msg.key.fromMe) {
    try {
      // Compute admin/owner status once — reused by both shields
      const [_shieldSenderIsAdmin, _shieldBotIsAdmin] = await Promise.all([
        isAdmin(sock, sender, from, groupMetadata),
        isBotAdmin(sock, from, groupMetadata)
      ]);
      const _shieldSenderIsOwner = isOwner(sender);

      const shieldOpts = {
        sender,
        from,
        senderIsAdmin:  _shieldSenderIsAdmin,
        senderIsOwner:  _shieldSenderIsOwner,
        botIsAdmin:     _shieldBotIsAdmin,
      };

      // Run both shields concurrently — isolated so one crash never stops the other
      await Promise.allSettled([
        handleCompetitorBot(sock, msg, groupMetadata, shieldOpts),
        handleNsfwShield(sock, msg, groupMetadata, shieldOpts),
      ]);
    } catch (_) {
      // Silent — shields never crash the main message handler
    }
  }
  // ================== END ALWAYS-ON SHIELDS ==================
    ...
    // ================== 🖕 ANTI-MIDDLE FINGER SHIELD ==================
    // Auto-detect and remove middle finger emoji from non-admin/non-owner users
    if (isGroup && !msg.key.fromMe) {
      try {
        const middleFingerEmojis = ['🖕', '🖕🏻', '🖕🏼', '🖕🏽', '🖕🏾', '🖕🏿'];
        let hasMiddleFinger = false;

        // Check text body
        if (body) {
          hasMiddleFinger = middleFingerEmojis.some(e => body.includes(e));
        }

        // Check sticker emoji metadata
        if (!hasMiddleFinger && content && content.stickerMessage) {
          const stickerEmoji = content.stickerMessage.associatedEmoji || '';
          hasMiddleFinger = middleFingerEmojis.some(e => stickerEmoji.includes(e));
        }
        ...
        if (hasMiddleFinger) {
          ...
          if (!senderIsAdmin && !senderIsOwner) {
            // Auto-reply warning
            await sock.sendMessage(from, {
              text: `*@${sender.split('@')[0]}, 🖕 is not allowed by 𝐌𝐫.𝐌𝐮𝐧𝐞𝐞𝐛 𝐀𝐥𝐢 𝐁𝐨𝐭!*`,
              mentions: [sender]
            }, { quoted: msg });

            // Auto-delete the offending message
            try {
        ...
        // Anti-middle-finger errors must never interrupt message processing
      }
    }
    // ================== 🖕 END ANTI-MIDDLE FINGER SHIELD ==================

    // ================== 📢 AUTO-MENTION REPLY (Owner Offline Guard) ==================
    // Fires whenever someone mentions the Owner's number and the Owner is offline.
    if (!msg.key.fromMe) {
      try {
        // Collect all mentioned JIDs from every possible wrapper
        const mentionedJids =
        ...
        const ownerPhone = '923329838699';
        const ownerMentioned = mentionedJids.some(jid => {
          const num = (jid || '').split('@')[0].split(':')[0];
          return num === ownerPhone;
        });

        if (ownerMentioned) {
          // Determine if owner is currently online (seen within last 5 minutes)
          const ownerIsOnline = (Date.now() - ownerLastSeenOnline) < OWNER_ONLINE_TTL;

          if (!ownerIsOnline) {
            const autoMentionReplies = [
              '𝗕𝗢𝗦𝗦 𝗕𝗨𝗦𝗬 𝗛𝗔𝗜𝗡🙁',
              '𝗠𝗥. 𝗠𝗨𝗡𝗘𝗘𝗕 𝗔𝗟𝗜 𝗔𝗕𝗛𝗜 𝗢𝗙𝗙𝗟𝗜𝗡𝗘 𝗛𝗔𝗜𝗡 𝗥𝗘𝗣𝗟𝗬 𝗕𝗔𝗔𝗗 𝗠𝗘𝗜𝗡 𝗠𝗜𝗟𝗘𝗚𝗔⚡',
              '𝗕𝗢𝗦𝗦 𝗜𝗦 𝗡𝗢𝗧 𝗛𝗘𝗥𝗘 𝗥𝗜𝗚𝗛𝗧 𝗡𝗢𝗪 𝗪𝗔𝗜𝗧 𝗞𝗔𝗥𝗘𝗜𝗡👑',
              '𝗕𝗢𝗦𝗦 𝗞𝗔𝗛𝗜𝗡 𝗕𝗨𝗦𝗬 𝗛𝗔𝗜𝗡 𝗠𝗘𝗦𝗦𝗔𝗚𝗘 𝗞𝗔𝗥 𝗗𝗘𝗜𝗡 𝗪𝗢𝗛 𝗗𝗘𝗞𝗛 𝗟𝗘𝗡𝗚𝗘⚠️'
            ];
            const selectedReply = autoMentionReplies[Math.floor(Math.random() * autoMentionReplies.length)];
            await sock.sendMessage(from, { text: selectedReply }, { quoted: msg });
          }
        }
      } catch (_) {
        // Auto-mention reply errors must never interrupt message processing
      }
    }
    // ================== 📢 END AUTO-MENTION REPLY ==================
  ...
  // ================== 🎉 AUTO-EMOJI ALIVE REPLY ==================
  // When a user sends a single emoji from the list, reply with a fun mention.
  if (!msg.key.fromMe) {
    try {
      const emojiReplyMap = {
        ...
        '😎': `@${sender.split('@')[0]} Swag toh check karo! Muneeb Ali ka bot hai, style toh hoga hi. 👑`,
      ...
      if (Object.prototype.hasOwnProperty.call(emojiReplyMap, trimmedBody)) {
        const replyText = emojiReplyMap[trimmedBody];
        await sock.sendMessage(from, { text: replyText, mentions: [sender] }, { quoted: msg });
        return;
      }
    } catch (_) {
      // Emoji reply errors must never interrupt message processing
    }
  }
  // ================== 🎉 END AUTO-EMOJI ALIVE REPLY ==================
...
// ── Owner Presence Tracker initializer ───────────────────────────────────────
// Call once after sock is connected. Listens for the owner's presence updates
// and records the last time they were seen as "available" or "composing".
// handleMessage uses `ownerLastSeenOnline` to decide if owner is offline.
const initializeOwnerPresenceTracker = async (sock) => {
  try {
    // Subscribe to the owner's presence updates
    await sock.presenceSubscribe(OWNER_JID);
  } catch (_) {}
  ...
  sock.ev.on('presence.update', ({ id, presences }) => {
    try {
      ...
      for (const [jid, presence] of Object.entries(presences)) {
        const num = (jid || '').split('@')[0].split(':')[0];
        if (num !== ownerPhone) continue;
        const status = presence.lastKnownPresence;
        // Mark as online for "available" or "composing" states
        if (status === 'available' || status === 'composing' || status === 'recording') {
          ownerLastSeenOnline = Date.now();
        }
      }
    } catch (_) {}
  });
